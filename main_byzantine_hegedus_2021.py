...
from byzantine_report import TokenizedGossipSimulatorByzantine
...
from byzantine_handler import RandomAttackMixin, AvoidReportMixin
...
class PartitionedTMHSameValueAttackHandler(RandomAttackMixin, PartitionedTMH, AvoidReportMixin):
    def __init__(self,
                 net: TorchModel,
                 ...
                 copy_model=True):
        RandomAttackMixin.__init__(self, 0.1)
...
topology = StaticP2PNetwork(100, to_numpy_matrix(
    random_regular_graph(20, 100, seed=42)))
...
nodes = generate_nodes(cls=PartitioningBasedNode,
                       ...
                       p2p_net=topology,
                       round_len=100,
                       model_proto=[(ceil(0.99 * dispatcher.size()), PartitionedTMH(
                           net=net,
                           ...
                           (floor(0.01 * dispatcher.size()), PartitionedTMHSameValueAttackHandler(
                               net=net,
                               tm_partition=TorchModelPartition(net, 4),
                               optimizer=torch.optim.SGD,
                               optimizer_params={
...
simulator = TokenizedGossipSimulatorByzantine(
    nodes=nodes,
    data_dispatcher=dispatcher,
